...
import 'package:mini_app/register.dart';
...
class _LoginpageState extends State<Loginpage> {
  ...
  Widget build(BuildContext context) {
    ...
    logindata() async{
      ...
      try{
        final result=await supabase.auth.signInWithPassword(email: email.text, password: passward.text);
        if(result !=null && result.session !=null){
          Navigator.push(context, MaterialPageRoute(builder: (context){
            return MyHomePage(title: "Your's Data");
          }));
        }
      }catch(e){
        print(e.toString());
   ...
   return Scaffold(
     appBar: AppBar(
       ...
       title: Center(child: Text("Login to Start Know",style: TextStyle(fontSize: 27, fontWeight: FontWeight.w800, color: Colors.white),)),
         ...
         children: [
           ...
           ElevatedButton(onPressed: (){
             logindata();
               ...
               ), child: Text("Login",style: TextStyle(fontWeight: FontWeight.w700,color: Colors.white),)),
           SizedBox(height: 21,),
